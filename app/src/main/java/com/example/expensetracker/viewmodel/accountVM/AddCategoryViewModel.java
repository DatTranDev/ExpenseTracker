...
public class AddCategoryViewModel extends BaseObservable {
    ...
    public  AddCategoryViewModel(Context context)
    {
        ...
        String categoryString = sharedPreferences.getString("category","null");
        categories = new MutableLiveData<>(getCategoriesFromSharedPreferences());

    }
    public void addCategory(){
        Log.e("Test: ", "Success");
        if(nameCategory.get()==null || nameCategory.get().equals("") || user==null)
        {
            ...
            try
            {
                CategoryReq categoryReq = new CategoryReq(user.getId(), nameCategory.get().toString(), "66143014c9f024002b0dabfa", parentCategory.get().getId(),type.get().toString(),false );
                Log.e("parent",parentCategory.get().getName().toString());
    ...
    private List<Category> getCategoriesFromSharedPreferences() {
